using CoreLib.Networking;
using RealTimeUdpStream.CoreLib.Models;
using RealTimeUdpStream.CoreLib.Networking;
using System;
...
namespace UI.Graphics
{
    internal class ScreenSender
    {
        ...
        public ScreenSender(string clientIp, int clientPort, int serverPort = 12000)
        {
            _server = new UdpPeer(serverPort);
            _clientEndPoint = new IPEndPoint(IPAddress.Parse(clientIp), clientPort);

            // Thiết lập chất lượng nén JPEG
        ...
        public async Task SendScreenLoopAsync()
        {
            uint sequenceNumber = 0; // Tăng dần theo mỗi khung hình

            while (true) // Vòng lặp stream vô tận
            {
                try
                {
                    byte[] largeData;
